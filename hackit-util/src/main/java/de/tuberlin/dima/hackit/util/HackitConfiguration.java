...
public abstract class HackitConfiguration{
  ...
  /**
   * Set of factory that will be use during the execution
   */
  Map<Class<?>, HackitFactory<?>> factories;
  ...
  /**
   * Add factory to the configurations
   *
   * @param factory is an instance of {@link HackitFactory}
   * @return self instance of HackitConfiguration
   * @param <T> type generated by the factory
   */
  public <T> HackitConfiguration addFactory(HackitFactory<T> factory){
    if(this.factories.containsKey(factory.getTClass())){
      throw new HackitException(
          String.format(
              "the class [%s] is already registered in the configuration",
              factory.getTClass()
          )
      );
    }
    this.factories.put(factory.getTClass(), factory);
    return this;
  }

  /**
   * Retrive the {@link HackitFactory} linked to the <code>tClass</code>
   *
   * @param tClass key to retrive the {@link HackitFactory}
   * @return {@link HackitFactory} instance
   * @param <T> type that will be created by the {@link HackitFactory}
   */
  public <T> HackitFactory<T> getFactory(Class<T> tClass){
    HackitFactory<T> factory = (HackitFactory<T>) this.factories.get(tClass);
    if(factory != null){
      return factory;
    }
    throw new HackitException(
        String.format(
            "The factory for [%s] does not exist",
            tClass.getName()
        )
    );
  }
